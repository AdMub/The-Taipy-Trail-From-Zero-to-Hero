...
company_names = filter_company_names(country)
...
def on_change(state, name, value):
    if name == "country":
        print(f"Country was changed to: {value}")
        # Update country in the scenario
        state.scenario.country.write(value)
        # Trigger task execution
        state.scenario.submit(wait=True)
        # Update company names dynamically
        state.company_names = state.scenario.company_names.read()
    if name == "company":
        print(f"Company was changed to: {value}")
...
if __name__ == "__main__":
    ...
    tp.Orchestrator().run()  # Starts the orchestrator service

    # Create a scenario instance
    scenario = tp.create_scenario(scenario_cfg)
    # Write the initial country to the scenario
